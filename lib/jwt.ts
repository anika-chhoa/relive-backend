...
export function setAuthCookie(res: Response, token: string) {
  res.cookie(COOKIE_NAME, token, {
    httpOnly: true,
    // Cross-site cookie (frontend on vercel.app, backend on onrender.com)
    // requires secure: true and sameSite: "none" — browsers will silently
    // drop the cookie otherwise. This forces HTTPS-only, which is fine
    // since both Render and Vercel serve over HTTPS.
    secure: true,
    sameSite: "none",
    maxAge: 7 * 24 * 60 * 60 * 1000,
    path: "/",
